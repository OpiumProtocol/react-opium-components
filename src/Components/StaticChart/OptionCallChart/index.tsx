...
const CustomTooltip = ({ active, payload, chartData1, chartData2, coordinate }: {coordinate?: any, active?: boolean, payload?: any, chartData1?: ChartData, chartData2?: ChartData}) => {
      ...
      <div className="custom-tooltip static-tooltip">
        {tooltips.map((chartData, i) => {
          return chartData && (<div
            ...
            style={{ backgroundColor: payload[i + 1].color, marginTop: i == 2 ? Math.abs(payload[2].value) * 10 : 0 }}
          >
            <p className="label">
  ...
  return (
    <ResponsiveContainer width='100%' height="100%">
      <ComposedChart data={dataWithZeros} margin={{ top: 25, right: 30, left: 20, bottom: 5 }} >
        ...
        <ReferenceLine stroke="white" strokeWidth="1" segment={[{ x: 15, y: -1.3 }, { x: 15, y: 1.3 }]} />
        ...
        <ReferenceDot r={3} stroke="none" x={15} y={0} label={<ReferenceRectDot value={'Strike price'} top={25} width={140}/>}/>
        <ReferenceDot r={3} stroke="none" x={22} y={0} label={<ReferenceRectDot value={'Break-even point'} top={-15} width={140} topY={-30} />}/>
      </ComposedChart>
    </ResponsiveContainer>
  )
}
