    ...
    if (active) {
      return (
        <div className="custom-tooltip">
          {payload && payload[0] && <p className="label cumulative">{`cumulative: ${payload[0].value}%`}</p>}
          {payload && payload[1] && <p className="label performance">{`performance: ${payload[1].value}%`}</p>}
        </div>
      )
    }
